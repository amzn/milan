...
import com.amazon.milan.flink.compiler.internal.{JoinLineageRecordFactory, RuntimeCompiledFunction, RuntimeCompiledFunction2, SerializableFunction, SerializableFunction2}
 ...
 */
class LatestApplyCoProcessFunction[TLeft <: Record, TRight <: Record, TKey, TOut <: Record](dateExtractor: SerializableFunction[TRight, Instant],
  ...
  override def processElement1(leftValue: TLeft,
    ...
    val lineageRecord = this.createLineageRecord(applyResult.getRecordId, leftValue)
  ...
  private def createLineageRecord(outputRecordId: String, leftRecord: TLeft): LineageRecord = {
    ...
    val sourceRecords = Seq(this.lineageRecordFactory.createLeftRecordPointer(leftRecord.getRecordId))
