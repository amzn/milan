...
object KeySelectorExtractor {
  ...
  def getKeyTupleFunctions(joinExpressionFunction: FunctionDef): (FunctionDef, FunctionDef) = {
    ...
    val leftArgName = joinExpressionFunction.arguments.head
    val leftTupleFunction = FunctionDef(List(leftArgName), Tuple(leftElements))

    val rightArgName = joinExpressionFunction.arguments.last
    val rightTupleFunction = FunctionDef(List(rightArgName), Tuple(rightElements))
  ...
  private def getKeyElementExpressions(keyFunction: FunctionDef): (List[Tree], List[Tree]) = {
    ...
    // two input streams.
    keyFunction match {
      case FunctionDef(List(leftArgName, rightArgName), body) =>
        this.getKeyElementExpressions(leftArgName, rightArgName, body)

      case _ =>
        throw new FlinkCompilationException(s"Unsupported key expression: $keyFunction")
    }
  }
    ...
    expr match {
      ...
      case Equals(left, right) =>
        ...
        if (leftTreeArgName == leftArgName && rightTreeArgName == rightArgName) {
          ...
          throw new FlinkCompilationException(s"Key expression references arguments '$leftTreeArgName' and '$rightTreeArgName', which do not match expected arguments '$leftArgName' and '$rightArgName'.")
        }

      case _ =>
        throw new FlinkCompilationException(s"Unsupported key expression: $expr")
    }
  }
  ...
  private def getReferencedArgument(expr: Tree): String = {
    ...
    if (termNames.size == 1) {
      ...
      throw new FlinkCompilationException(s"More than one argument is referenced in join key element expression: $expr")
    }
  }
}
