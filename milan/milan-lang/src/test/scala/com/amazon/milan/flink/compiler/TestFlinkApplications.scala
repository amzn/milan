...
class TestFlinkApplications {
  ...
  def test_FlinkApplication_WithMappedTupleToRecordStream_OutputsMappedRecords(): Unit = {
    ...
    val outputStream = tupleStream.map { case Tuple1(i) => IntRecord(i) }
  ...
  @Test
  def test_FlinkApplication_WithTupleStreamProjectOnto_OutputsExpectedRecords(): Unit = {
    val input = Stream.of[IntKeyValueRecord]
    val tuple = input.map(
      ((r: IntKeyValueRecord) => r.key) as "key",
      ((r: IntKeyValueRecord) => r.value) as "value")
    val output = tuple.projectOnto[IntKeyValueRecord]

    val graph = new StreamGraph(output)

    val config = new ApplicationConfiguration()
    val inputRecords = List(IntKeyValueRecord(1, 2), IntKeyValueRecord(3, 4))
    ...
    env.executeThenWaitFor(() => sink.getRecordCount == 2, 10)

    val outputRecords = sink.getValues.sortBy(_.key)
    assertEquals(inputRecords, outputRecords)
  }
  ...
  def test_FlinkApplication_WithTupleStreamAddField_OutputsRecordsWithAddedField(): Unit = {
    ...
    val sink = config.addMemorySink(output)(output.getRecordType)
