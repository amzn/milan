...
import com.amazon.milan.compiler.flink.runtime.{DelayedListSourceFunction, WrapRecordsMapFunction}
...
trait DataSourceGenerator {
  ...
  private def addListDataSource(outputs: GeneratorOutputs,
    ...
    val code =
      q"""
         ...
         |val $valuesVal = com.amazon.milan.compiler.flink.runtime.RuntimeUtil.loadJsonList[${recordType.toTerm}]($sourceJson)
         |val $dataStreamVal = com.amazon.milan.compiler.flink.runtime.DataSourceUtil.addListDataSource[${recordType.toTerm}](
  ...
  private def addFileDataSource(outputs: GeneratorOutputs,
    ...
    val code =
      q"""
         |val $recordTypeInfoVal = ${liftTypeDescriptorToTypeInformation(recordType)}
         |val $dataStreamVal = com.amazon.milan.compiler.flink.runtime.DataSourceUtil.addFileDataSource[${recordType.toTerm}](
         |  ${outputs.streamEnvVal},
         |  ${source.path},
         |  ${source.dataFormat},
  ...
  private def addDelayedListDataSource(outputs: GeneratorOutputs,
    ...
    val codeBlock =
      q"""
         |val $recordTypeInfoVal = ${liftTypeDescriptorToTypeInformation(recordType)}
         |val $valuesVal = com.amazon.milan.compiler.flink.runtime.RuntimeUtil.loadJsonList[$itemTypeName]($sourceJson)
  ...
  private def addKinesisDataSource(outputs: GeneratorOutputs,
    ...
    val code =
      q"""val $recordTypeInfoVal = ${liftTypeDescriptorToTypeInformation(recordType)}
         |val $dataStreamVal = com.amazon.milan.compiler.flink.runtime.KinesisDataSource.addDataSource[${recordType.toTerm}](
         |  ${outputs.streamEnvVal},
         |  ${source.streamName},
         |  ${source.region},
         |  ${source.dataFormat},
