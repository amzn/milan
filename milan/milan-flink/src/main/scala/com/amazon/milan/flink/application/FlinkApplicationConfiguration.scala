...
class FlinkApplicationConfiguration(var dataSources: Map[String, FlinkDataSource[_]],
   ...
   */
  def setSource[T](stream: Stream[T], source: FlinkDataSource[T]): Unit =
   ...
   */
  def addSink[T](stream: Stream[T], sink: FlinkDataSink[T]): Unit =
   ...
   */
  def addMetricDefinition[T](stream: Stream[T], metricDefinition: MetricDefinition[T]): Unit =
