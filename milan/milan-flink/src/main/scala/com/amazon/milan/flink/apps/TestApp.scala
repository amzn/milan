...
object TestApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val recordType = TypeDescriptor.create[TestAppRecord](
      "com.amazon.milan.flink.apps.TestAppRecord",
      List(
        (RecordIdFieldName, types.String),
        ("value", types.Int)))

    val typeDesc = types.stream(recordType)
    ...
    val inputStream = new Stream[TestAppRecord](streamNode, recordType)
...
class TestAppRecord(val recordId: String, val value: Int) {
  def this() {
    this(newId(), 0)
  }
