...
import com.amazon.milan.program.{Aggregate, Cycle, ExternalStream, Filter, FlatMap, GroupBy, GroupingExpression, JoinExpression, Last, LeftWindowedJoin, ScanExpression, SelectTerm, SlidingRecordWindow, StreamExpression, StreamMap, TimeWindowExpression, Tree, Union}
...
class GeneratorContext(val applicationInstanceId: String,
  ...
  private val components =
    new Object
      ...
      val typeLifter: TypeLifter = contextTypeLifter
    }
  ...
  private def generateMapExpression(mapExpr: StreamMap): GeneratedDataStream = {
    mapExpr.source match {
      ...
      case recordWindowExpr: SlidingRecordWindow =>
        this.generateApplyWindow(mapExpr, recordWindowExpr)
  ...
  private def generateApplyWindow(mapExpr: StreamMap,
                                  windowExpr: SlidingRecordWindow): GeneratedDataStream = {
    val inputStream = this.getOrGenerateStream(windowExpr.source)

    inputStream match {
      case keyedStream: GeneratedKeyedDataStream =>
        this.components.applyRecordWindowApplyOfKeyedStream(this, keyedStream, mapExpr, windowExpr)
    }
  }
