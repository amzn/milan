...
object FlinkMapFunctionFactory {
   ...
   * @return A [[SingleOutputStreamOperator]] of the mapped records and their associated lineage records.
   */
  def applyMapToRecordFunction[TIn, TOut](mapExpr: MapRecord,
    ...
    val inputRecordType = mapExpr.getInputRecordType.asInstanceOf[TypeDescriptor[TIn]]
    val outputRecordType = mapExpr.recordType.asInstanceOf[TypeDescriptor[TOut]]

    val lineageWrapper = new LineageMapFunctionWrapper[TIn, TOut, MapToRecordMapFunction[TIn, TOut]](
      inputRecordType,
      outputRecordType,
      flinkMapFunction,
      lineageFactory)
  ...
  def applyMapToFieldsFunction[TIn](mapExpr: MapFields,
                                    inputDataStream: SingleOutputStreamOperator[TIn],
                                    metricFactory: MetricFactory,
                                    lineageFactory: LineageRecordFactory,
                                    outputTypeInformation: TupleStreamTypeInformation): SingleOutputStreamOperator[RecordWithLineage[ArrayRecord]] = {
    val inputRecordType = mapExpr.getInputRecordType.asInstanceOf[TypeDescriptor[TIn]]
    val outputRecordType = mapExpr.recordType.asInstanceOf[TypeDescriptor[ArrayRecord]]
    ...
    val lineageWrapper = new LineageMapFunctionWrapper[TIn, ArrayRecord, MapToFieldsMapFunction[TIn]](
      inputRecordType,
      outputRecordType,
      flinkMapFunction,
      lineageFactory)
