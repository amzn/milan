...
abstract class LeftJoinCoProcessFunction[TLeft, TRight, TOut](leftInputType: TypeDescriptor[TLeft],
      ...
      if (output != null) {
        val lineage =
          if (this.canProduceLineage) {
            this.createLineageRecord(this.getOutputRecordId.get(output), leftValue, rightValue)
          }
          else {
            null
          }
  ...
  private def createLineageRecord(outputRecordId: String, leftRecord: TLeft, rightRecord: TRight): LineageRecord = {
    val sourceRecords =
      Option(leftRecord).toSeq.map(r => this.lineageRecordFactory.createLeftRecordPointer(this.getLeftRecordId.get(r))) ++
        Option(rightRecord).toSeq.map(r => this.lineageRecordFactory.createRightRecordPointer(this.getRightRecordId.get(r)))
...
class LeftJoinMapToRecordCoProcessFunction[TLeft, TRight, TOut](mapExpr: MapRecord,
...
class LeftJoinMapToFieldsCoProcessFunction[TLeft, TRight](mapExpr: MapFields,
                                                          leftInputType: TypeDescriptor[TLeft],
                                                          rightInputType: TypeDescriptor[TRight],
                                                          rightTypeInformation: TypeInformation[TRight],
