...
import com.amazon.milan.storage.{BlobStore, IO}
...
object S3BlobStore {
   ...
   */
  def create(region: Region, bucketName: String, rootFolder: String): S3BlobStore = {
...
class S3BlobStore(s3ClientFactory: S3ClientFactory, bucketName: String, rootFolder: String) extends BlobStore {
  ...
  override def readBlob(key: String, targetStream: OutputStream): Unit = {
    ...
    val request = GetObjectRequest.builder().bucket(this.bucketName).key(fullKey).build()
    val s3Object = this.s3Client.getObject(request)
    IOUtils.copy(s3Object, targetStream)
  }
  ...
  override def writeBlob(key: String, blobStream: InputStream): Unit = {
    ...
    val result = this.s3Client.putObject(request, RequestBody.fromBytes(bytes))

    this.logger.info(s"Blob copy complete (ETag = ${result.eTag()}).")
