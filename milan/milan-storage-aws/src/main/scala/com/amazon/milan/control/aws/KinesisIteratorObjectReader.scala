...
object KinesisIteratorObjectReader {
  ...
  def forShard[T: ClassTag](streamName: String,
                            shardId: String,
                            region: Region = Region.EU_WEST_1): KinesisIteratorObjectReader[T] = {
   ...
   * @return A [[KinesisIteratorObjectReader]] that yields objects from the records.
   */
  def forOnlyShard[T: ClassTag](streamName: String, region: Region = Region.EU_WEST_1): KinesisIteratorObjectReader[T] = {
    val client = KinesisClient.builder()
      .credentialsProvider(DefaultCredentialsProvider.create())
      .region(region)
      .build()
   ...
   * @return A [[KinesisIteratorObjectReader]] that yields objects from the records.
   */
  def forStreamWithOneShard[T: ClassTag](client: KinesisClient, streamName: String): KinesisIteratorObjectReader[T] = {
    ...
    if (!this.hasNext) {
      ...
      if (nextRecord == null) {
        ...
        nextRecord.map(record => {
          val data = record.data().asByteArray()
